    ...
    options.uri = url // options 에 uri 값을 url 값으로 변경해줌
        ...
        return new Promise(resolve=>{ // 비동기사용하여 info return
            ...
            request(options,(err,response,body)=>{
                ...
                }else if(body === undefined){//만약 body undefined라면
...
function imgsDownload({mainDir,title,chapterList,imgs,i,url,cdnList}){ // 이미지 다운로드 함수
    ...
    const savedirList = `${mainDir}/${title}/${chapterList[i]}` // 폴더 이름 및 경로 지정해주기


    if(!fs.existsSync(savedirList)){ //폴더가 없다면 폴더 생성
        ...
        for(let c = 0; c < imgs.length; c++){
            ...
            let changedImg = imgs[c].split('"')[1]+'?quick' // url 변환해줌

            changedImg = changedImg.replace(/\\/g, "") // \\역슬래쉬 제거


            const chk = new RegExp("filecdn.xyz");


            if(chk.test(changedImg)){ //filecdn 포함되있다면 changedImg 를 cdnList 로 변환해줌

              changedImg = changedImg.replace(chk,cdnList[c]).replace('"','').replace('"','')

            }

            //다운로드함수 인데 알규먼트들이 너무 많다 나중에한번 싹 수정해줘야겠다
...
function imgsLoad(info,options,mainDir){// 이미지들을 불러와주는함수
    ...
    const ids = info.list.ids // 아이디값들
        ...
        for(let i = 0; i < ids.length; i++){
            ...
            setTimeout(()=>{ //다운로드 속도 지정 하기 위해

                const id = ids[i]

                const url = changeUrl(viewUrl,id) // id 값 url 변환

                options.uri = url //options 에 uri url 변환해주기
                ...
                request(options,(err,response,body)=>{
                    ...
                    if(body.split('var img_list = ')[1]){ //var img_list 직접 바디에 추출


                        let imgs = body.split('var img_list = ')[1].split(';')[0].split(','); // var img_list 추출
                        let cdnList = body.split('var cdn_domains =')[1].split(';')[0].split(',') // cdnList 추출 보호된 이미지 다운로드

                            imgsDownload({mainDir //디렉토리
                                ,title, //제목
                                chapterList, //리스트
                                imgs //이미지
                                ,i // for i 변수
            ...
            },DOWNLOAD_SPEED*500*i) // 다운로드 스피드 지정 해주기
